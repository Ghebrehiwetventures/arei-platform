...
import { GuideSocialBuilderView } from "./GuideSocialBuilderView";
...
function NorthStarBanner() {
  // Collapsible disclosure. Default collapsed so AREI Pulse leads the
  // dashboard; the full strategic copy stays one click away.
  const [open, setOpen] = useState(false);

  return (
    <div className="rounded border border-border bg-surface-2 px-5 py-3">
      {/* Collapsed strip: label + hero tagline + expand control. */}
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      >
        <div className="min-w-0">
          <p className="text-[10px] font-mono font-medium uppercase tracking-wider text-foreground-subtle">North Star</p>
        ...
        <span className="shrink-0 text-[10px] font-mono uppercase tracking-wider text-foreground-subtle hover:text-foreground-muted">
          {open ? "Collapse ▲" : "Expand ▼"}
        </span>
      </button>

      {/* Expanded content: descriptor, What / How / Why, footer. */}
      {open && (
        <div className="mt-3">
...
type Tab = "dashboard" | "listings" | "sources" | "chatlab" | "agencies" | "agency-data" | "broker-pilot" | "market-news" | "marketing" | "guide-social-builder" | "news-studio" | "news-clusters" | "source-health" | "notifications" | "featured" | "briefings" | "reports" | "reports-marketing";
...
const MARKETING_NAV_ITEMS: { key: Tab; label: string }[] = [
  { key: "guide-social-builder", label: "Guide Social Builder" },
      ...
      <main className="flex-1 min-w-0 overflow-x-hidden overflow-y-auto bg-background">
        ...
        <div className="max-w-[1200px] mx-auto px-4 py-5 md:px-8 md:py-8">
          <MarketProvider>
            ...
            {tab === "guide-social-builder" && <GuideSocialBuilderView />}
