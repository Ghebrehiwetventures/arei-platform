...
export async function searchPexelsPhoto() {
  // Runtime uses the curated allowlist ONLY — a fixed, human-verified set of
  // Cape Verde photos — so a Pexels background is always genuinely Cape Verde.
  // Live keyword search is deliberately NOT used at render time: for a small
  // island nation it kept surfacing look-alike neighbours (Canary Islands,
  // West Africa). When the allowlist is empty, return null so the caller falls
  // back to the AI image — never a wrong-country photo.
  //
  // Populate / extend the allowlist via /api/pexels-curate (which still runs a
  // live, human-reviewed Pexels search). Uploaded images are the other real-
  // photo path.
