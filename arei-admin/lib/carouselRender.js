...
function headlineBlock({ text, x, lastBaseline, fontSize, lineHeight, weight, accent, maxWidth }) {
  const words = String(text || "").trim().split(/\s+/).filter(Boolean);
  ...
  const svg = lines.map((lineWords, li) => {
    const spans = lineWords.map(() => {
      const idx = wi++;
      return `<tspan fill="${aset.has(idx) ? SAGE : BONE}">${esc(words[idx])} </tspan>`;
    }).join("");
...
const kicker = (x, y, text) =>
  `<text x="${x}" y="${y}" font-family="Inter" font-size="22" font-weight="600" letter-spacing="3.5" fill="${SAGE}">${esc(text)}</text>`;

const counter = (x, y, idx, total) =>
  `<text x="${x}" y="${y}" font-family="Inter" font-size="17" font-weight="600" letter-spacing="2" fill="${BONE}" text-anchor="end">${idx} / ${total}</text>`;
...
export async function renderSlide(slide) {
  ...
  const { W, H, M, lockH, bandH, bottomSafe } = fmt;
  const innerW = W - 2 * M;
  const hasPhoto = Boolean(slide.imageBuffer);

  // Photo background (cover / listing) or solid ink (statement / priceCheck /
  ...
  if (slide.type === "listing") {
    ...
    bg = `<rect width="${W}" height="${H}" fill="#16384a"/>
      ...
      <rect width="${W}" height="240" fill="url(#t)"/>
      <rect width="${W}" height="${H}" fill="url(#b)"/>`;
  } else {
    bg = `<rect width="${W}" height="${H}" fill="${INK}"/>`;
  }
  ...
  if (slide.type === "listing") {
    ...
    const hb = headlineBlock({ text: slide.title, x: M, lastBaseline: last, fontSize: fit.fontSize, lineHeight: Math.round(fit.fontSize * 1.04), weight: 600, accent: slide.accent, maxWidth: innerW });
    body = `${lockup(M, M, BONE, lockH)}
      ${kicker(M, hb.topBaseline - fit.fontSize - 26, slide.kicker || "// CABO VERDE")}
      ${hb.svg}
      ${slide.dek ? `<text x="${M}" y="${H - bottomSafe}" font-family="Inter" font-size="28" font-weight="400" fill="${DIM}">${esc(slide.dek)}</text>` : ""}`;
    ...
    const hb = headlineBlock({ text: slide.text, x: M, lastBaseline: H - bottomSafe, fontSize: fit.fontSize, lineHeight: Math.round(fit.fontSize * 1.12), weight: 600, accent: slide.accent, maxWidth: innerW });
    body = `${lockup(M, M, BONE, lockH)}
      ${kicker(M, hb.topBaseline - fit.fontSize - 26, slide.kicker || (slide.type === "priceCheck" ? "// PRICE CHECK" : "// THE MOMENT"))}
      ${hb.svg}`;
    ...
    const hb = headlineBlock({ text: slide.title || "See what it actually costs.", x: M, lastBaseline: subY - 60, fontSize: fit.fontSize, lineHeight: Math.round(fit.fontSize * 1.06), weight: 600, accent: slide.accent, maxWidth: innerW });
    body = `${lockup(M, M, BONE, lockH)}
      ${kicker(M, hb.topBaseline - fit.fontSize - 26, slide.kicker || "// THE INDEX")}
      ${hb.svg}
      <text x="${M}" y="${subY}" font-family="Inter" font-size="27" font-weight="400" fill="${DIM}">${esc(slide.sub || "Live Cape Verde listings · structured property data.")}</text>
      <text x="${M}" y="${urlY}" font-family="Inter" font-size="24" font-weight="600" letter-spacing="0.5" fill="${BONE}">${esc(slide.url || "capeverderealestateindex.com")}</text>`;
  } else {
