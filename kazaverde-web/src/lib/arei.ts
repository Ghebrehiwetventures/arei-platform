...
function getClient(): AREIClient {
  ...
  if (!url || !key) {
    const msg =
      "Missing Supabase config. Copy kazaverde-web/.env.example to .env and set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.";
...
export const arei = {
  ...
  subscribeNewsletter: (email: string, locale?: string) => getClient().subscribeNewsletter(email, locale),
