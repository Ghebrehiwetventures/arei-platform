...
// URL + ID utilities live in ./fetcher/parse/url so strategy modules can reuse
// them without dragging in the whole orchestrator.
import {
  applyDetailUrlRewrite,
  extractIdFromUrl,
  generateListingId,
  makeAbsoluteUrl,
} from "./fetcher/parse/url";

// Image filtering + WP-size-aware dedup lives in ./fetcher/parse/images.
// dedupeImageUrls is re-exported to preserve the public API for existing
// callers (genericDetailExtractor, pipeline/enrich).
import {
  dedupeImageUrls,
  isValidImageUrl,
} from "./fetcher/parse/images";
export { dedupeImageUrls } from "./fetcher/parse/images";

// Price parsing lives in ./fetcher/parse/price.
import { parsePrice } from "./fetcher/parse/price";
