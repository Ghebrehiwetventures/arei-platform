...
// Preflight intentionally uses parseGenericHtml (single-page HTML probe) rather than
// genericPaginatedFetcher (full pagination + headless) — it only needs a quick sample.
...
function generateTimestampFilename(marketId: string): string {
  const now = new Date();
  ...
  return `${marketId}_preflight_${yyyy}${mm}${dd}_${hh}${min}${ss}.json`;
}
...
function persistReport(report: PreflightReport): string {
  ...
  const baseFilename = generateTimestampFilename(report.marketId);
...
export async function runPreflightMarket(marketId: string): Promise<PreflightReport> {
  const configResult = loadSourcesConfig(marketId);
  ...
  const summary = {
    ...
    inCount: results.filter((r) => r.lifecycleState === LifecycleState.IN).length,
    observeCount: results.filter((r) => r.lifecycleState === LifecycleState.OBSERVE).length,
    dropCount: results.filter((r) => r.lifecycleState === LifecycleState.DROP).length,
  };
  ...
  if (allImageQualityMetrics.length > 0) {
    ...
    const globalTierDist = { A: 0, B: 0, C: 0, D: 0 };
...
if (require.main === module) {
  ...
  if (!marketId) {
    ...
    console.error("Usage: MARKET_ID=cv npx ts-node --transpile-only core/preflightMarket.ts");
    process.exit(1);
  }

  runPreflightMarket(marketId)
    .then((report) => {
