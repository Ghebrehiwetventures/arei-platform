/**
 * Description-rewrite feasibility study v1.1.
 *
 * Strategic multi-source sample (Gabetti excluded — v1 already covered).
 * Same prompt, same dual-model setup as v1, same CSV format plus two
 * new manual columns: formatting_improvement_claude/gpt (1-3-5 + N/A).
 *
 * Source quotas (weighted by catalogue production share):
 *   cv_estatecv:            8   (largest source, ~38% of catalogue)
 *   cv_terracaboverde:      5
 *   cv_simplycapeverde:     4
 *   cv_homescasaverde:      4
 *   cv_capeverdeproperty24: 3   (chaotic formatting — structural cleanup test)
 *   cv_cabohouseproperty:   3   (longest text, likely most sales language)
 *   cv_oceanproperty24:     3
 *   total:                  30
 *
 * Filters: description IS NOT NULL AND LENGTH(description) > 500.
 * Within each source: stratify on property_type (with `_unknown` bucket).
 *
 * Run:
 *   ANTHROPIC_API_KEY=... OPENAI_API_KEY=... \
 *   npx ts-node --transpile-only scripts/experiments/description_rewrite_v1/run_v1_1.ts
 *
 * Flags:
...
// Same prompt as v1 — promptversionen bumpas trots oförändrad text
// per användarens spec, för att markera ny studie-iteration.
...
function parseArgs(): { smoke: boolean; dryRun: boolean } {
...
async function main(): Promise<void> {
  ...
  console.log(`[v1.1] run_id=${runId} prompt_version=${PROMPT_VERSION} smoke=${smoke} dry_run=${dryRun}`);

  // Per-source: fetch eligible candidates, stratify, take quota.
  ...
  console.log(`[v1.1] sample composition:`);
  ...
  console.log(`[v1.1] total picks=${picks.length}`);
  ...
  const results = await runWithConcurrency(
    ...
    (done, total) => {
      if (done % 5 === 0 || done === total) {
        console.log(`[v1.1] ${done}/${total} done`);
      }
    }
  );
  ...
  console.log(`[v1.1] wrote ${csvPath}`);
  console.log(`[v1.1] wrote ${metaPath}`);
  console.log(
    `[v1.1] elapsed=${elapsedSec}s  claude_errors=${meta.counts.claude_errors}  gpt_errors=${meta.counts.gpt_errors}`
  );
}
