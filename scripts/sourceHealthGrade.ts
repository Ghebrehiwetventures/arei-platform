 ...
 *
 * THIS FILE IS A DUPLICATE of arei-admin/sourceHealthGrade.ts.
 *
 * Why two copies:
 *   arei-admin/ is ESM ("type":"module"); scripts/ runs in CJS context
 *   (inherits from repo root). Bridging via dynamic import() works in Node
 *   but is fragile under ts-node. Cleanest is to keep identical copies and
 *   verify equality in CI.
 *
 * Used by:
 *   - arei-admin/data.ts (admin live display) — imports from arei-admin/
 *   - scripts/snapshot_source_health.ts (daily snapshot writer) — imports
 *     from this file.
 *
 * The Source-health-snapshot GitHub workflow runs a diff between the two
 * copies and fails the run if they differ. If you change one, change the
 * other.
 */
